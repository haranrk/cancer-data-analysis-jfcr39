...
class JointNMF_mask(object):
    ...
    def calc_distance_of_HW_to_X(self):
        ...
        self.diff = np.sum(np.sum(np.abs(self.X1r_pre-self.X1r))) + np.sum(np.sum(np.abs(self.X2r_pre-self.X2r))) + np.sum(np.sum(np.abs(self.X2r_pre-self.X2r)))
    ...
    def set_PanH(self):
        ...
        columnsPanH = ["X1_" + x for x in self.H1.columns] + ["X2_" + x for x in self.H2.columns] + ["X3_" + x for x in self.H3.columns]
